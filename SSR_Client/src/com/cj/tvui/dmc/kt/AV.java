...
public class AV implements AVInterface, VODInterface{
	...
	Player player = (Player) sc.getServiceContentHandlers()[0];
	AWTVideoSizeControl sizeCntr = ( AWTVideoSizeControl ) player.getControl( "javax.tv.media.AWTVideoSizeControl" );
	 ...
	 * VOD Status
	 */
	private int vodStatus = VOD_STOP;
	...
	public void startVOD(String assetId) {
		...
		this.offsetTime = 0;
		String said = StbController.getInstance().getStbInterface().getSubscribeID();
		if(requestParam == null) requestParam = new ParamMap();
		...
		try {
			...
			if ( unicastService != null ) {
				...
				if (responseParam != null) {
					...
					if (listener != null) {
						listener.vodPlayerState(false, true, VOD_ERROR_1);
					}
				}
			} else {
