...
public final class KeyController{
    ...
    public boolean keyProcess(int keycode) {
    	...
    	if(!isEnableBackKey && parsedKeyCode == Keys.VK_BACK) {
    		return false;
    	}
    	if(!isEnableNumKey) {
    		switch(parsedKeyCode) {
    	...
    	Scene targetScene = SceneController.getInstance().getCurrentScene();
    	...
    	//targetScene.dispatchEvent(e);
    	
        targetScene._onKeyDown(parsedKeyCode);
